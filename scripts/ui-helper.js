      ...
      const pulseEl = qs("pulsePlayerContainer");
  ...
  <audio id="audio" preload="metadata" tabindex="-1">
    <source src="https://f005.backblazeb2.com/file/onestopmp3pi/file_example_MP3_700KB.mp3" type="audio/mpeg" />
    Your browser does not support the audio element.
  </audio>
</div>
...
<script>
  (function () {
    ...
    updatePlayPause();
      ...
      qs("#copyCode2").addEventListener("click", async () => {
        const code2 = pulseEl.textContent.trim();
        if (!code2) return;
